...
module uart_Tx_fixed #(
    ...
    // --- 1. RX Input Synchronizer ---
    reg rx_s1, rx_s2;
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            rx_s1 <= 1'b1; // Idle high
            rx_s2 <= 1'b1;
        end else begin
            rx_s1 <= rx;
            rx_s2 <= rx_s1;
    ...
    // --- 4. RX Logic (Async Reset + Synced Input) ---
        ...
        if (reset) begin
            ...
            case (rx_state)
                RX_IDLE: begin
                    ...
                    if (!rx_s2) rx_state <= RX_START; // Use Synced Input
                end
                RX_START: begin
