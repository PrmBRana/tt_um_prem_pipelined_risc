...
module uart_Tx_fixed0 #(
    ...
    // --- Baud Rate Generator (Async Reset) ---
    ...
    // --- TX Logic (Async Reset) ---
    reg [7:0]  tx_buf;
    reg [3:0]  tx_bit_idx;
    reg [3:0]  tx_os_cnt; // Optimized width
    reg        tx_active;

    always @(posedge clk or posedge reset) begin
        if (reset) begin
            tx <= 1'b1; tx_busy <= 1'b0; tx_active <= 1'b0;
            tx_buf <= 8'd0; tx_bit_idx <= 4'd0; tx_os_cnt <= 0;
        end else begin
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            rx_sync <= 2'b11; rx_active <= 1'b0; rx_bit_idx <= 4'd0;
            rx_os_cnt <= 0; rx_shift <= 8'd0; rx_Data <= 8'd0; rx_ready <= 1'b0;
        end else begin
            ...
            if (!rx_active) begin
                if (rx_sync[1] == 1'b0) begin
                    rx_active <= 1'b1; rx_bit_idx <= 4'd0;
                    rx_os_cnt <= 0;    rx_shift   <= 8'd0;
                end
            end else if (baud_tick) begin
                if (rx_os_cnt == OVERSAMPLE/2) begin
                    case (rx_bit_idx)
